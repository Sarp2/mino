...
/** Tracks hovered and selected elements. Handles click, shift+click (multi-select), and delete (with safety checks for component roots, body, and dynamic elements). */
export class ElementsManager {
    ...
    mouseover(domEl: DomElement) {
        ...
        if (this._hovered?.domId && this._hovered.domId === domEl.domId) return;
        ...
        this.editorEngine.overlay.state.updateHoverRect(
            adjustedRect,
            isComponent,
        );
    ...
    emitError(error: string) {
        console.error(error);
        toast.error('Cannot dlete element', { description: error });
    }
    ...
    async delete() {
        ...
        for (const selectedEl of selected) {
            ...
            if (!frameData?.view) {
                console.error('No iframe view found');
                return;
            }
            ...
            const removeAction: RemoveElementAction | null =
                await frameData.view.getRemoveAction(selectedEl.domId, frameId);

            if (!removeAction) {
                this.emitError(
                    'Remove action not found. Try refreshing the page',
                );
                return;
            }
            ...
            // TODO: Uncomment it out when BranchManager is done
    ...
    private async shouldDelete(
        ...
        frameData: FrameData,
    ): Promise<{ shouldDelete: boolean; error?: string }> {
        ...
        if (!instanceId) {
            ...
            const result = await frameData.view.getElementType(
                selectedEl.domId,
            );
