...
/** Handles drag-to-reorder and keyboard reorder (up/down) for elements. Uses a preparation phase (150ms + 10px threshold) to distinguish clicks from drags. Creates MoveElementActions on drop so changes are undoable. */
export class MoveManager {
    ...
    private dragPrepartionTimer: ReturnType<typeof setTimeout> | null = null;
    ...
    startDragPrepartion(
        ...
        if (this.dragPrepartionTimer) {
            clearTimeout(this.dragPrepartionTimer);
        }
        ...
        this.dragPrepartionTimer = setTimeout(() => {
            void (async () => {
                if (this.isPreparing) {
                    await this.prepareDrag(el, frameData);
                }
                this.dragPrepartionTimer = null;
            })();
    ...
    async drag(
        ...
        const frameData = this.editorEngine.frames.get(
            this.state.dragTarget.frameId,
        );
        ...
        try {
            ...
            const positionType =
                this.state.dragTarget.styles?.computed?.position;
    ...
    async shiftElement(
        ...
        try {
            ...
            const currentIndex = await frameData.view.getElementIndex(
                element.domId,
            );
            ...
            const childrenCount = await frameData.view.getChildrenCount(
                parent.domId,
            );
