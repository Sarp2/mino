...
export interface NavigationOject {
    history: string[];
    currentIndex: number;
}

/** Tracks per-iframe page navigation history (back/forward) like a browser's address bar. */
export class FrameNavigationManager {
    private frameIdToNavigationObject = new Map<string, NavigationOject>();
    ...
    canGoBack(frameId: string): boolean {
        ...
        if (!navigationObject) return false;
    ...
    addToHistory(frameId: string, path: string): void {
        const navigationObject = this.frameIdToNavigationObject.get(
            frameId,
        ) ?? {
            history: [],
            currentIndex: 0,
        ...
        if (
            navigationObject.currentIndex <
            navigationObject.history.length - 1
        ) {
        ...
        // Add new path to history if it is not the same as the previous path
    ...
    goBack(frameId: string): string | null {
        if (!this.canGoBack(frameId)) return null;
        ...
        if (
            previousIndex < 0 ||
            previousIndex >= navigationObject.history.length
        ) {
            return null;
        }
    ...
    goForward(frameId: string): string | null {
        if (!this.canGoForward(frameId)) return null;
