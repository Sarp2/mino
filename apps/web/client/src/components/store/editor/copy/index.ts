...
/** Handles copy, paste, cut, and duplicate for elements. Stores a full element
 *  snapshot + code block on copy, assigns fresh IDs on paste, and smartly chooses
 *  to insert as sibling or child based on the paste target. */
export class CopyManager {
    ...
    async copy() {
        ...
        const targetEl: ActionElement | null =
            await frameData.view.getActionElement(selectedEl.domId);
    ...
    async clearClipboard() {
        try {
            ...
            console.warn('Failed to clear clipbopard: ', error);
        }
    }
    ...
    async getInsertLocation(
        ...
        if (insertAsSibling) {
            const location: ActionLocation | null =
                await frameData.view.getActionLocation(selectedEl.domId);
