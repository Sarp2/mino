...
import { convertFontStrong } from '@mino/utility';
...
/** Reads styles from selected elements for the style panel, and writes style changes back through ActionManager. Supports Root mode (edit component definition) and Instance mode (edit single instance only). */
export class StyleManager {
    ...
    init() {
        this.selectedElementsReactionDisposer = reaction(
            ...
            (selectedElements) =>
                this.onSelectedElementsChanged(selectedElements),
        );
    }
            ...
            targets: action.targets.map((val) => ({
                ...
                change: {
                    original: Object.fromEntries(
                        Object.entries(val.change.original).map(
                            ([key, styleChange]) => [
                                key,
                                {
                                    ...styleChange,
                                    value: convertFontStrong(styleChange.value),
                                },
                            ],
                        ),
                    ),
                    updated: Object.fromEntries(
                        Object.entries(val.change.updated).map(
                            ([key, styleChange]) => [
                                key,
                                {
                                    ...styleChange,
    ...
    updateStyleNoAction(styles: CSSProperties) {
        ...
        if (this.selectedStyle === null) return;
    ...
    private onSelectedElementsChanged(selectedElements: DomElement[]) {
        ...
        for (const selectedEl of selectedElements) {
            const selectedStyle: SelectedStyle = {
                styles:
                    selectedEl.styles ??
                    ({ defined: {}, computed: {} } as DomElementStyles),
