...
import type { Frame, LayerNode } from '@mino/models';
import type { EditorEngine } from '../engine';

/** Handles events coming from iframes (DOM Mutations, resizes, layer tree updates) and monitor wheter frames are visible in the viewport for the "recenter" prompt */
export class FrameEventManager {
    ...
    private undebouncedViewportCheck = () => {
        ...
        const isAnyFrameInView = frames.some((frame) =>
            this.isFrameInViewPort(frame.frame),
        );
    ...
    recenterCanvas = () => {
        ...
        if (firstFrame) {
            ...
            const frameCenterX =
                firstFrame.position.x + firstFrame.dimension.width / 2;
            const frameCenterY =
                firstFrame.position.y + firstFrame.dimension.height / 2;
