...
import type { IframeView } from '../frames';

import { adaptRectToCanvas } from '../overlay/utils';

/** Manages inline text editing lifecycle - start (open ProseMirror editor), edit (sync keystrokes to iframe), end (commit). Groups all keystrokes into a single history transaction for one-step undo. */
export class TextEditingManager {
    ...
    async start(el: DomElement, frameView: IframeView): Promise<void> {
        try {
            const isEditable = await frameView.isChildTextEditable(
                el.oid ?? '',
            );
            if (!isEditable) {
                throw new Error(
                    isEditable === null
                        ...
                        : "Can't edit text because it is not plain text. Edit in code or use AI.",
                );
            }
            ...
            if (!res) {
                throw new Error(
                    'Failed to start editing text, no result returned',
                );
            }
            ...
            if (!computedStyles) {
                throw new Error(
                    'Failed to get computed styles for text editing.',
                );
            }
            ...
            // TODO: Uncomment it out when HistoryManager is done
    ...
    async end(): Promise<void> {
        try {
            ...
            const res = await frameData.view.stopEditingText(
                this.targetDomEl.domId,
            );
            if (!res) {
                throw new Error(
                    'Failed to stop editing text. No result returned',
                );
            }
    ...
    async editSelectedElement(): Promise<void> {
        if (this.shouldNotStartEditing) return;
