import type { ElementPosition, RectDimensions } from '@mino/models';
...
export function getRelativeOffset(element: HTMLElement, ancestor: HTMLElement) {
    ...
    while (currentElement && currentElement !== ancestor) {
        ...
        if (!offsetParent || offsetParent === ancestor) break;
...
export function adaptRectToCanvas(
    ...
    return {
        ...
        left:
            (rect.left + sourceOffset.left + canvasTransform.e / scale) * scale,
    };
}
