...
/** Determines whether a container's children flow vertically or horizontally by comparing the positions of its first two children */
...
/** Returns the insertion index for a flex/block layout by finding the first sibling whose midpoint is past the cursor position. */
...
/** Returns the insertion index for a grid layout by mapping the cursor position to a grid cell using column/row counts. */
export function findGridInsertionIndex(
    ...
    return Math.min(Math.max(targetIndex, 0), sibling.length);
}
