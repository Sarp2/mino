...
export class CodeSandboxProvider extends Provider {
    ...
    async reload(): Promise<boolean> {
        ...
        const task = this.client?.tasks.get('dev');
        if (task) {
            await task.restart();
            return true;
        }
    ...
    async downloadFiles(
        ...
        return { url: res.downloadUrl };
    }

    async copyFiles(input: CopyFilesInput): Promise<CopyFilesOutput> {
        if (!this.client) {
            throw new Error(
                'Client not initialized. There is no WebSocket connection',
            );
    ...
    async watchFiles(input: WatchFilesInput): Promise<WatchFilesOutput> {
        ...
        if (input.onFileChange) {
            watcher.registerCallback(async (event) => {
                if (input.onFileChange) {
    ...
    async runCommand({
        args,
    }: TerminalCommandInput): Promise<TerminalCommandOutput> {
        if (!this.client) {
            throw new Error(
                'Client not initialized. There is no WebSocket connection',
            );
    ...
    async runBackgroundCommand(
        ...
        const command = await this.client.commands.runBackground(
            input.args.command,
        );
...
export class CodeSandboxFileWatcher extends ProviderFileWatcher {
    ...
    registerCallback(
        ...
        this.watcher.onEvent((event) => {
            void Promise.resolve(callback(event)).catch((error: unknown) => {
                console.error('watch callback failed', error);
            });
        });
    }
